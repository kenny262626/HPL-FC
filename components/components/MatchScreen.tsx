...
export default function MatchScreen({ canEdit, onNavigate, showToast }: Props) {
  ...
  function addPhoto() {
    if (!edit || !photoInput.trim()) return;
    setEdit({ ...edit, photos: [...edit.photos, photoInput.trim()] });
    setPhotoInput('');
  }
      ...
      <div className="screen-scroll">
        <div style={{padding:'16px 24px',display:'flex',flexDirection:'column',gap:16}}>
          {matches.map((m) => {
            ...
            const res = resultLabel(m.score_us || 0, m.score_them || 0);
            return (
              <div key={m.id} style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden'}}>
                ...
                <div style={{background:'linear-gradient(135deg,#1a1600,#151200)',padding:'14px 16px',borderBottom:'1px solid rgba(232,200,74,0.2)',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:'var(--sub)',marginTop:2}}>
                      {m.place && `${m.place} · `}{m.start_time}{m.end_time ? ` – ${m.end_time}` : ''}{m.method && ` · ${m.method}`}
                    </div>
                  </div>
                  {canEdit && !isEditing && (
                    <button onClick={() => openEdit(m)}
                      style={{background:'none',border:'1px solid var(--border)',borderRadius:8,padding:'4px 8px',cursor:'pointer',color:'var(--sub)',fontSize:16,lineHeight:1}}>
                      ⚙️
                    </button>
                  )}
                </div>
                ...
                {isEditing && edit && (
                  <div style={{padding:'16px',display:'flex',flexDirection:'column',gap:10,borderBottom:'1px solid var(--border)'}}>
                    ...
                    {/* 출석 멤버 */}
                    <div>
                      <div style={{fontSize:10,color:'var(--sub)',marginBottom:6}}>출석 멤버</div>
                    ...
                    {/* 사진 URL */}
                    <div>
                      <div style={{fontSize:10,color:'var(--sub)',marginBottom:6}}>사진 URL</div>
                      <div style={{display:'flex',gap:6}}>
                        <input value={photoInput} onChange={e=>setPhotoInput(e.target.value)} placeholder="https://..." className="admin-input" style={{flex:1}}/>
                        <button onClick={addPhoto} style={{background:'var(--card2)',border:'1px solid var(--border)',borderRadius:8,padding:'0 12px',color:'var(--accent)',cursor:'pointer',fontSize:18,lineHeight:1}}>+</button>
                      </div>
                        ...
                        {edit.photos.map((url, i) => (
                          <div key={i} style={{position:'relative'}}>
                            ...
                            <button onClick={() => removePhoto(i)}
                              style={{position:'absolute',top:-4,right:-4,background:'var(--red)',border:'none',borderRadius:'50%',width:16,height:16,color:'#fff',cursor:'pointer',fontSize:10,lineHeight:'16px',textAlign:'center',padding:0}}>×</button>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {/* 스코어 표시 */}
                {!isEditing && (
                  <div style={{padding:'14px 16px',display:'flex',alignItems:'center',justifyContent:'center',gap:12}}>
                    <span style={{fontFamily:'Bebas Neue,sans-serif',fontSize:36,color:'var(--green)'}}>{m.score_us ?? '-'}</span>
                    <div style={{textAlign:'center'}}>
                      <div style={{fontSize:16,color:'var(--sub)'}}>:</div>
                      <div style={{fontFamily:'Bebas Neue,sans-serif',fontSize:18,color:res.color,marginTop:2}}>{res.text}</div>
                    </div>
                ...
                {!isEditing && m.photos && m.photos.length > 0 && (
                  <div style={{padding:'0 16px 14px',display:'flex',gap:8,overflowX:'auto'}}>
                    {m.photos.map((url, i) => (
                      <img key={i} src={url} alt="" style={{width:80,height:80,objectFit:'cover',borderRadius:10,flexShrink:0}}/>
                    ))}
                  </div>
                )}
